...
def json_to_simpson(dataset, path_and_name, sim_options, sim_options_num, sim_options_str): 
    ...
    main_file = open(os.path.join(data_path, data_name + "_simps.in"), 'w')
    spinsys_file = open(os.path.join(data_path, data_name + "_simps.spinsys"), 'w')
        ...
        for ms in magres['ms']:
            if ms.has_key('mview_data'):
                try:
                    ...
                    continue
                spinsys_file.write(" shift\t" + str(i) + " " + str(-ms['mview_data'][0]) + "p " + str(-2.0/3.0*ms['mview_data'][1])                 # The factor is because Simpson uses reduced anisotropy
    ...
    main_file.write("source " + os.path.join(data_path, data_name + "_simps.spinsys") + "\n\n")
