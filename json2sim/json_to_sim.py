...
def json_to_sim(datafile, sim_options, sim_options_num, sim_options_str):
    ...
    data_path = os.path.abspath(os.path.join(os.getcwd(), os.path.split(datafile)[0]))
    data_name = os.path.split(datafile)[1].split('.', 1)[0]
    in_file.close()
...
#       return False
    ...
    if dataset.has_key('soft_targ') == False:
        ...
        json_to_simpson(dataset, (data_path, data_name), sim_options, sim_options_num, sim_options_str)
    elif dataset['soft_targ'] == 'spinev':
        json_to_spinev(dataset, (data_path, data_name), sim_options, sim_options_num, sim_options_str)
    else:
