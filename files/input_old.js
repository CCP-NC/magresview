...
//This variable holds the last successfully loaded file
...
function parseAtomBlock(mag_parser)
{
	...
	while(!mag_parser.prs_end())
	{
		...
		switch(spl_line[0])
		{
			...
			case "lattice":
				if (spl_line.length < 10 || atom_set.units["lattice"] == null)
					return null;
...
//		magres_parser(this.result);
		...
		load_script += "{" + x_lat + " " + y_lat + " " + z_lat + "} packed";

		Jmol.script(mainJmol, load_script);

		last_file = file_content;
	};

	jmol_rdr.readAsText(to_load);
}

//Data extraction from Jmol

function jmoldata_extract()
{

	//1. Find out species, elements, and labels. This comes from a getPropertyAsArray on AtomInfo

	//2. Find out actual atomic coordinates. We run an evaluate_each for that purpose

	//3. Find out ms data - if present

	//4. Find out efg data - if present

	//5. Find out isc data - if present
